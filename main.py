...
if __name__ == '__main__':
    ...
    for rounds in range(args.T):
        ...
        if args.server_method == 'fedqr':
            client_acc_list = []
            print("\n=== FedQR Client accuracy ===")
            for idx in select_list:
                acc = validate(args, client_nodes[idx])
                client_acc_list.append(acc)
                print(f"Client {idx} accuracy: {acc:.4f}")
            avg_acc = sum(client_acc_list) / len(client_acc_list)
            print(f"This round client average accuracy: {avg_acc:.4f}")
            test_acc_recorder.append(avg_acc)
            fedqr_avg_acc_history.append(avg_acc)
            current_best_avg = max(fedqr_avg_acc_history)
            print(f"best average accuracy: {current_best_avg:.4f}")
            test_acc_recorder.append(avg_acc)
        else:
            acc = validate(args, central_node, which_dataset='local')
            print(f"{args.server_method}{args.client_method}, Global model accuracy: {acc}")
            test_acc_recorder.append(acc)

        if args.server_method != 'fedqr':
            print("weight vectors:", size_weights)
