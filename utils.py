...
def pad(tensor_larger, tensor_smaller, value):
    padding_list = []
    padding = tensor_larger.shape[-1] - tensor_smaller.shape[-1]
    padding_before = padding // 2
    padding_after = padding - padding_before
    padding_list.extend([padding_before, padding_after])
    tensor_padded = F.pad(tensor_smaller, pad=padding_list, mode='constant', value=value)
    return tensor_padded

def qr_2d(tensor_2d):
    q, r = torch.linalg.qr(tensor_2d)
    min_dim = min(r.shape[0], r.shape[1])
    diag_elements = r.diag()[:min_dim]
    diag_sign = torch.sign(diag_elements)
    diag_sign[diag_sign == 0] = 1

    q_corrected = q.clone()
    q_corrected[:, :min_dim] = q[:, :min_dim] * diag_sign.unsqueeze(0)

    r_corrected = r.clone()
    r_corrected[:min_dim, :] = r[:min_dim, :] * diag_sign.unsqueeze(1)
    r_corrected[:, :min_dim] = r_corrected[:, :min_dim] * diag_sign.unsqueeze(0)

    if q_corrected.shape[-1] < tensor_2d.shape[-1]:
        q_corrected = pad(tensor_2d, q_corrected, 0)
    return q_corrected, r_corrected

def qr_2d_no_padding(tensor_2d):
    q, r = torch.linalg.qr(tensor_2d)
    min_dim = min(q.shape[1], r.shape[0], r.shape[1])
